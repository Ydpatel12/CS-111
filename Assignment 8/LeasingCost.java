public class LeasingCost {
	...
	public static Vehicle createVehicle(String description){
        ...
        String nameBeginnning = description.substring(description.indexOf("name:") + 5); //vehicle name
        String name = nameBeginnning.substring(0, nameBeginnning.indexOf(";"));
        
        
        String fuelTypeBeginning = description.substring(description.indexOf("type:") + 5); //Fueltype
        String fuelType = fuelTypeBeginning.substring(0, fuelTypeBeginning.indexOf(";"));
        ...
        if (fuelType.equals("gas")){
            String usageBeginning = description.substring(description.indexOf("mile/unit:") + 10); //Usage
            double usage = Double.parseDouble(usageBeginning.substring(0, usageBeginning.indexOf(";")));
            fuel = new Fuel (usage);
        }
        ...
        {
            String chargerCostBeginning = description.substring(description.indexOf("charger:") + 8); // ChargerCost
            double chargerCost = Double.parseDouble(chargerCostBeginning.substring(0, chargerCostBeginning.indexOf(";")));

            String usageBeginning = description.substring(description.indexOf("mile/unit:") + 10); //Usage
            double usage = Double.parseDouble(usageBeginning.substring(0, usageBeginning.indexOf(";")));
            fuel = new Fuel (usage, chargerCost);
        }


        String dueAtSigningBeginning = description.substring(description.indexOf("due:") + 4); //DueAtSigning
        double dueAtSigning = Double.parseDouble(dueAtSigningBeginning.substring(0,dueAtSigningBeginning.indexOf(";")));

        String leaseLengthBeginning = description.substring(description.indexOf("length:") + 7); //LeaseLength
        int leaseLength = Integer.parseInt(leaseLengthBeginning.substring(0,leaseLengthBeginning.indexOf(";")));


        String monthlyCostBeginning = description.substring(description.indexOf("monthly:") + 8); //MonthlyCost
        double monthlyCost = Double.parseDouble(monthlyCostBeginning.substring(0,monthlyCostBeginning.indexOf(";")));


        String mileageAllowanceBeginning = description.substring(description.indexOf("allowance:") + 10);//MileageAllowance
        int mileageAllowance = Integer.parseInt(mileageAllowanceBeginning.substring(0,mileageAllowanceBeginning.indexOf(";")));
        ...
        for(int i = 0; i < vehicles.length; i++){ 
            if(vehicles[i].getFuel().getType() == Fuel.GAS){
                ...
                vehicles[i].setTotalCost(vehicles[i].getFuelCost() + computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost()));
                 ...
                 vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease().getLeaseLength(), vehicles[i].getFuel().getUsage(), vehicles[i].getLease().getMileageAllowance(), .453 ));
                 ...
                 vehicles[i].setTotalCost(vehicles[i].getFuelCost() + computeLeaseCost(vehicles[i].getLease().getDueAtSigning(), vehicles[i].getLease().getLeaseLength(), vehicles[i].getLease().getMonthlyCost()) + vehicles[i].getFuel().getCharger());
	...
	public static void main (String[] args) {
		...
		for ( Vehicle v : vehicles ) 
            ...
            System.out.println("CO2EMITTED_ELECTRICITYCOMBUSTION");
    }
}
