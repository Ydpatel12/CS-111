//call populateLocationProbabilities in oneMonthForecast
//
...
public class WeatherGenerator {
    ...
    public static void populateLocationProbabilities( double[] drywetProbability, double[] wetwetProbability, 
        ...
        for (int i = 0; i < drywet.length; i++){
            if(drywet[i][0] == longitude && wetwet[i][1] == latitude){
                for(int j = 0; j < 12; j++){
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) {
        ...
        double percent = StdRandom.uniform();
        if(percent < .5){
            days[0] = WET;
        }
        else{
        ...
        for(int i = 1; i < numberOfDays; i++){
            double percent2 = StdRandom.uniform();
            int previousday = days[i-1];

            if(previousday == WET){
                if(percent2 <= wetwetProbability){
                    days[i] = WET;
                }
                else{
                    days[i] = DRY;
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for (int i = 0; i < forecast.length; i++){
                ...
                if(forecast.length - i <= 7){
                    week -= 1;
                }
                
                for(int j = i; j < i + week; j++){
                    if(forecast[j] == 2){
                        count++;
                }
            }
